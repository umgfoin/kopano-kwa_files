...
Zarafa.plugins.files.data.Actions = {
	...
	/**
	 * Upload the given files to the files backend. This method will use the legacy form submit to
	 ...
	 */
	uploadLegacyItems: function (files, form, store) {
		var destination;
		...
		// build the ids:
		var ids = [];
		Ext.each(files, function (filename) {
			var id = destination + Zarafa.plugins.files.data.Utils.File.getFileName(filename);
			ids.push({
				id      : id,
				isFolder: false
			});
		...
		container.getRequest().singleRequest(
			...
			new Zarafa.plugins.files.data.ResponseHandler({
				successCallback: this.checkForExistingFilesDone.createDelegate(this, [files, form, destination, this.doLegacyUpload], true)
			})
		);
	},

	/**
	 * Actually upload the files by submitting the form.
	 *
	 * @param button
	 * @param value
	 * @param options
	 ...
	 * @param destination
	 */
	doLegacyUpload: function (button, value, options, files, form, destination) {
		if (!Ext.isDefined(button) || button === "yes") {
			form.submit({
				waitMsg: dgettext('plugin_files', 'Uploading files') + '...',
				url    : 'index.php?sessionid=' + container.getUser().getSessionId() + '&load=custom&name=upload_file',
				params : {
					"parentID": destination
				},
				failure: function (form, action, success) {
					Zarafa.common.dialogs.MessageBox.show({
						title  : dgettext('plugin_files', 'Error'),
						msg    : dgettext('plugin_files', 'Uploading files failed!'),
						icon   : Zarafa.common.dialogs.MessageBox.ERROR,
						buttons: Zarafa.common.dialogs.MessageBox.OK
					});
				},
				success: function (form, action, success) {
					this.uploadDone(files, destination)
				},
				scope  : this
			});
		}
